  ...
  try {
    ...
    if (parseInt(rows[0].count) === 0) {
      ...
      const vendors = [
        ...
        { name: 'Bloom & Wild Florals',    cat: 'Florals',        email: 'hello@bloomwild.co',       phone: '+1 555-0202', status: 'active',    notes: 'Premium florals.' },
        { name: 'Metro Catering Co.',      cat: 'Catering',       email: 'orders@metrocatering.com', phone: '+1 555-0303', status: 'pending',   notes: 'Awaiting cert renewal.' },
        ...
        { name: 'EliteTransport Group',    cat: 'Transportation', email: 'fleet@elitetransport.net', phone: '+1 555-0606', status: 'pending',   notes: 'Negotiating rates.' },
      ];
      const vendorIds = [];
      for (const v of vendors) {
        const id = uuid(); vendorIds.push(id);
        await client.query(
      ...
      for (const e of events) {
        const id = uuid(); eventIds.push(id);
        await client.query(
...
app.listen(PORT, async () => {
  console.log(`VendorOS API running on port ${PORT} [${process.env.NODE_ENV || 'development'}]`);
  await initDatabase();
});
